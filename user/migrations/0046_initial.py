# Generated by Django 3.2.6 on 2021-08-22 23:48

import django.contrib.postgres.fields
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('user', '0045_auto_20210822_2347'),
    ]

    operations = [
        migrations.CreateModel(
            name='Booking',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('totalprice', models.IntegerField()),
                ('nbuses', models.IntegerField()),
            ],
        ),
        migrations.CreateModel(
            name='Bus',
            fields=[
                ('name', models.CharField(max_length=1000)),
                ('busid', models.IntegerField(primary_key=True, serialize=False)),
                ('buscategory', models.CharField(max_length=100)),
                ('bustype', models.CharField(max_length=100)),
                ('regno', models.CharField(max_length=30)),
                ('latideal', models.FloatField()),
                ('lngideal', models.FloatField()),
                ('affiliationlevel', models.CharField(max_length=100)),
                ('havegps', models.IntegerField()),
                ('latreal', models.FloatField()),
                ('lngreal', models.FloatField()),
                ('lastupdated', models.TimeField()),
            ],
        ),
        migrations.CreateModel(
            name='BusInStop',
            fields=[
                ('businstopid', models.CharField(default='1', max_length=200, primary_key=True, serialize=False)),
                ('getint', models.DateTimeField()),
                ('getoutt', models.DateTimeField()),
                ('finaldesttime', models.DateTimeField()),
                ('ithstop', models.IntegerField()),
            ],
        ),
        migrations.CreateModel(
            name='Conductor',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('conductorid', models.IntegerField()),
                ('name', models.CharField(max_length=1000)),
                ('phone', models.CharField(max_length=30)),
                ('passhash', models.CharField(max_length=1000)),
            ],
        ),
        migrations.CreateModel(
            name='Connect',
            fields=[
                ('fromgetint', models.DateTimeField()),
                ('togetint', models.DateTimeField()),
                ('fromgetoutt', models.DateTimeField()),
                ('togetoutt', models.DateTimeField()),
                ('connectid', models.CharField(max_length=1000, primary_key=True, serialize=False)),
                ('price', models.IntegerField(default=99)),
            ],
        ),
        migrations.CreateModel(
            name='Place',
            fields=[
                ('gplaceid', models.CharField(default='0', max_length=500)),
                ('placename', models.CharField(max_length=1000)),
                ('popularity', models.IntegerField()),
                ('lat', models.FloatField()),
                ('lng', models.FloatField()),
                ('stoptype', models.CharField(max_length=100)),
                ('placeid', models.IntegerField(primary_key=True, serialize=False)),
                ('district', models.CharField(default='UNKNOWN', max_length=200)),
                ('state', models.CharField(default='Kerala', max_length=200)),
                ('departadd', models.ManyToManyField(related_name='_user_place_departadd_+', through='user.Connect', to='user.Place')),
            ],
        ),
        migrations.CreateModel(
            name='Route',
            fields=[
                ('origt', models.DateTimeField()),
                ('destt', models.DateTimeField()),
                ('getints', django.contrib.postgres.fields.ArrayField(base_field=models.DateTimeField(), size=None)),
                ('getoutts', django.contrib.postgres.fields.ArrayField(base_field=models.DateTimeField(), size=None)),
                ('routeid', models.IntegerField(primary_key=True, serialize=False)),
                ('bus', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='routes', to='user.bus')),
                ('conductor', models.ForeignKey(on_delete=models.SET('UNASSIGNED'), to='user.conductor')),
                ('dest', models.ForeignKey(on_delete=models.SET('DELETED'), related_name='routedestplace', to='user.place')),
                ('orig', models.ForeignKey(on_delete=models.SET('DELETED'), related_name='routeorigplace', to='user.place')),
                ('stopsadd', models.ManyToManyField(through='user.BusInStop', to='user.Place')),
            ],
        ),
        migrations.CreateModel(
            name='SingleSearchResult',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('getint', models.DateTimeField()),
                ('getoutt', models.DateTimeField()),
                ('price', models.IntegerField()),
                ('getinp', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='resultgetinp', to='user.place')),
                ('getoutp', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='resultgetoutp', to='user.place')),
                ('route', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='user.route')),
            ],
        ),
        migrations.CreateModel(
            name='Owner',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=1000)),
                ('phone', models.CharField(max_length=30)),
                ('address', models.CharField(max_length=3000)),
                ('passhash', models.CharField(max_length=1000)),
                ('ownerid', models.IntegerField()),
                ('ownedbuses', models.ManyToManyField(related_name='ownerownedbusesbus', to='user.Bus')),
            ],
        ),
        migrations.CreateModel(
            name='MKUser',
            fields=[
                ('name', models.CharField(max_length=1000)),
                ('uid', models.CharField(max_length=200, primary_key=True, serialize=False)),
                ('phone', models.CharField(max_length=30)),
                ('type', models.CharField(max_length=10)),
                ('logincreds', models.CharField(max_length=1000)),
                ('bookings', models.ManyToManyField(to='user.Booking')),
            ],
        ),
        migrations.AddField(
            model_name='connect',
            name='getinp',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='departures', to='user.place'),
        ),
        migrations.AddField(
            model_name='connect',
            name='getoutp',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='arrivals', to='user.place'),
        ),
        migrations.AddField(
            model_name='connect',
            name='route',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='connects', to='user.route'),
        ),
        migrations.AddField(
            model_name='conductor',
            name='employer',
            field=models.ForeignKey(on_delete=models.SET('DELETED'), to='user.owner'),
        ),
        migrations.AddField(
            model_name='conductor',
            name='presentbus',
            field=models.ManyToManyField(related_name='conductorpresentbus', to='user.Bus'),
        ),
        migrations.AddField(
            model_name='businstop',
            name='finaldest',
            field=models.ForeignKey(default=1, on_delete=django.db.models.deletion.CASCADE, related_name='finaldest', to='user.place'),
        ),
        migrations.AddField(
            model_name='businstop',
            name='place',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='routes', to='user.place'),
        ),
        migrations.AddField(
            model_name='businstop',
            name='route',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='stops', to='user.route'),
        ),
        migrations.AddField(
            model_name='bus',
            name='owner',
            field=models.ForeignKey(on_delete=models.SET('DELETED'), to='user.owner'),
        ),
        migrations.AddField(
            model_name='bus',
            name='presentconductor',
            field=models.ForeignKey(on_delete=models.SET('UNASSIGNED'), to='user.conductor'),
        ),
        migrations.AddField(
            model_name='booking',
            name='dest',
            field=models.ForeignKey(on_delete=models.SET('DELETED'), related_name='bookingdest', to='user.place'),
        ),
        migrations.AddField(
            model_name='booking',
            name='origin',
            field=models.ForeignKey(on_delete=models.SET('DELETED'), related_name='bookingorigin', to='user.place'),
        ),
        migrations.AddField(
            model_name='booking',
            name='user',
            field=models.ForeignKey(on_delete=models.SET('DELETED'), to='user.mkuser'),
        ),
    ]
